...
class Exportfile {
  ...
  public Exportfile(PApplet parent_, String filename_, Mask mask_) {
    ...
    PShape masklaserfile = loadShape("../data/cut/" + filename);
  ...
  void load_laserfiles() {
    ...
    String directory = "../data/cut/"+filename+"/UI/";
  ...
  void create_pdf_files() {
    ...
    PGraphicsPDF pdf = (PGraphicsPDF) createGraphics(1600*displayDensity(), 900*displayDensity(), PDF, "data/output/"+pdfname+"_output.pdf");
    pdf.beginDraw();
    pdf.background(255);
    ...
    for (int i=0; i<laserfiles.size(); i++) {
      ...
      if (design) {
        ...
        if (i == 0) {
          pdf.textAlign(CENTER);
          pdf.text("Place base acrylic sheet in the laser cutter", 800, 135); 
          pdf.text("Place a sheet of " + cellophane_t + " cellophane in the laser cutter", 800, 165);
        } else {
          pdf.textAlign(CENTER);
          pdf.text("Place a sheet of " + cellophane_t + " cellophane in the laser cutter", 800, 135);
        }
        pdf.fill(0);
        pdf.textAlign(CENTER);
        pdf.text("Power: 12.5% \nSpeed: 3% \nLaser z: "+laserz+ " mm", 800, 800);
